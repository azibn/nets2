...
def plot_metrics(cnn, seed):
    ...
    # Bottom left: Validation predictions colored by original labels
    sc = axes[1, 0].scatter(
    ...
    axes[1, 0].set_xlabel('Tpeak [BJD - 2457000]')
    axes[1, 0].set_ylabel('Probability of Exocomet')
    axes[1, 0].set_title('Probabilities (with the original labels)')
    axes[1, 0].legend()
    plt.colorbar(sc, ax=axes[1, 0], ticks=np.arange(4), boundaries=np.arange(4+1)-0.5)
    
    # Bottom right: Binary classification (gt)
    ...
    plt.savefig(f"plots/cnn-metrics-s{seed}.png", dpi=300, bbox_inches='tight')
    plt.close()
