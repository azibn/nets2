...
def process_file(file_path):
    try:
        ...
        if np.min(data) < 0:
            ...
            return (
                f"Plotted {os.path.basename(file_path)} - minimum value: {np.min(data)}"
            )
...
def process_numpy_files(directory):
    ...
    with ProcessPoolExecutor() as executor:
        # Submit all tasks
        future_to_file = {
            executor.submit(process_file, file): file for file in npy_files
        }

        # Process results as they complete with a progress bar
        for future in tqdm(
            as_completed(future_to_file), total=len(npy_files), desc="Processing files"
        ):
