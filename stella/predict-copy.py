    ...
    try:
        try:
            if path.endswith(".fits"):
                ...
                time = arr[0]
                flux = arr[1]
                flux_error = flux / np.nanmedian(flux) * 0.1
                #flux_error = arr[2] if len(arr) > 2 else np.zeros_like(flux)
                source_id = os.path.basename(path).split('_')[0]  # or another way to get ID
        ...
        return source_id, time, flux, flux_error, original_flux, original_time
