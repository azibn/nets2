...
duration_range = np.logspace(np.log10(min_duration), np.log10(max_duration), num_duration_points) ## logspace
...
def generate_models(n_models=args.n, save=args.s, save_path=args.sp,types=args.types):
    ...
    if types == 'noiseless':
        for i, (skewness, duration, model_index) in tqdm.tqdm(enumerate(itertools.product(skew, duration_range, range(n_models)))):
            time = np.load("time-array-s7.npy")
            duration = np.round(duration,2)
            flux = skewed_gaussian(time, alpha=skewness, t0=1500, sigma=duration, A=0.001)
            ...
            if save:
                ...
                file_path = os.path.join(folder_path, f"exocomet_model_{model_index}.npy")
    ...
    for skewness in skew:
        for duration in duration_range:
            for snr in snr_range:
                for _ in tqdm.tqdm(range(n_models), desc=f"Skew: {skewness}, Duration: {duration:.2f}, SNR: {snr}"):
                    lightcurve = np.random.choice(lightcurves)  # Randomly choose a lightcurve for each model
                    lc, lc_info = import_lightcurve(lightcurve, drop_bad_points=True)
                    duration = np.round(duration, 2)
                    time, flux, fluxerror, model = exocomet(lc=lc, lc_info=lc_info, snr=snr, duration=duration, t0=1496.5, folder='exocomet-grid-models', skew=skewness)
                    
                    if save:
                        cfolder = f"injected-skew_{skewness}-duration-{duration:.2f}-snr-{snr}"
                        folder_path = os.path.join(save_path, cfolder)
                        os.makedirs(folder_path, exist_ok=True)
                        file_path = os.path.join(folder_path, f"exocomet_model_{lc_info['TIC_ID']}_sector07.npy")
...
def exocomet(lc,lc_info,snr,duration,t0=1500,folder='exocomet-grid-models',skew=skew):
